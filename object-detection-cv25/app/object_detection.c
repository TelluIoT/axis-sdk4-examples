...
// BEGIN BBOX GLOBALS
#define BOUNDING_BOX_DISPLAY_COUNT   10
#define BOUNDING_BOX_GREEN_THRESHOLD 0.2
...
//     if (!bbox)
//         syslog(LOG_INFO, "Failed creating: %s", strerror(errno));

//     // If camera lacks video output, this call will succeed but not do anything.
//     if (!bbox_video_output(bbox, true))
//         syslog(LOG_INFO, "Failed enabling video-output: %s", strerror(errno));
...
//     // Draw all queued geometry simultaneously
//     if (!bbox_commit(bbox, 0u))
//         syslog(LOG_INFO, "Failed committing: %s", strerror(errno));

//     syslog(LOG_INFO, "BOUNDING BOX DRAWN. SLEEPING");
//     g_usleep(5 * 1000000);
...
static box* sort_object_bounding_boxes(box* orig_boxes, size_t length) {
    gboolean swapped;

    box* boxes = malloc(sizeof(box) * length);
    memcpy(boxes, orig_boxes, sizeof(box) * length);
...
static void draw_object_bounding_boxes(box* unsorted_object_boxes,
                                       size_t object_boxes_length,
    ...
    // bbox_coordinates_frame_normalized(bounding_box);  // TODO: not sure if this should be used
    ...
    syslog(LOG_INFO, "About to draw boxes: got %zd boxes", object_boxes_length);

    box* object_boxes = sort_object_bounding_boxes(unsorted_object_boxes, object_boxes_length);

    size_t box_count = 0;
    size_t i         = 0;
    while (i < object_boxes_length && box_count < 5) {  // TODO: fix
        box* object_box = &object_boxes[i++];
        char* label     = labels[object_box->label - 1];

        if (object_box->label == 0 || object_box->score < 0.1) {
            continue;
            syslog(LOG_INFO, "GOT BOX WITH LABEL 0");
        }
        ...
        float top    = object_box->y_min;
        float left   = object_box->x_min;
        float bottom = object_box->y_max;
        float right  = object_box->x_max;

        unsigned int cropped_frame_width = frame_height;

        // Get screen coords
        unsigned int crop_x = left * cropped_frame_width + (frame_width - frame_height) / 2;
        unsigned int crop_y = top * frame_height;
        unsigned int crop_w = (right - left) * cropped_frame_width;
        unsigned int crop_h = (bottom - top) * frame_height;

        // Normalize screen coords
        float box_left   = crop_x / (float)frame_width;
        float box_top    = crop_y / (float)frame_height;
        float box_right  = (crop_x + crop_w) / (float)frame_width;
        float box_bottom = (crop_y + crop_h) / (float)frame_height;

        // // Get color based on score
        // bbox_color_t color = object_box->score >= BOUNDING_BOX_GREEN_THRESHOLD
        //                          ? BOUNDING_BOX_COLOR_GREEN
        //                          : BOUNDING_BOX_COLOR_RED;

        // Get color based on label
        bbox_color_t color;
        if (strcmp(label, "bed") == 0) {
        ...
        syslog(LOG_INFO,
               "Drawing bounding with index %zd score %f, label %s, color: %p, crop x/y/w/h: %d %d "
               "%d %d, "
    ...
    //     const float x = (200u) * (i % 8u) / frame_width;
    //     const float y = (200u) * (i / 8u) / frame_height;

    //     // Switch color [This operation is fast!]
    //     bbox_color(bounding_box, BOUNDING_BOX_COLOR_GREEN);

    //     bbox_rectangle(bounding_box, x, y, x + 30, y + 30);
    // }
...
int main(int argc, char** argv) {
    ...
    // Seems to be set to 640x360 by default?
    const int desiredHDImgWidth  = args.raw_width;  // TODO: try changing to 1280
    const int desiredHDImgHeight = args.raw_height;
    const int threshold          = 20;  // args.threshold;
    ...
    syslog(
        LOG_INFO,
        "HDImgWidth/Height %dx%d input width/height %dx%d numdetections %d padding: %d classes: %d",
        desiredHDImgWidth,
        desiredHDImgHeight,
        inputWidth,
        inputHeight,
    ...
    int streamWidth  = 640;  // TODO: try changing to 1280?
    int streamHeight = 360;  // TODO: try changing to 720?
    syslog(LOG_INFO,
    ...
    while (true) {
        ...
        draw_object_bounding_boxes(boxes, numberOfDetections, widthFrameHD, heightFrameHD, labels);
        ...
        for (int i = 0; i < numberOfDetections; i++) {
            ...
            if (boxes[i].score >= threshold / 100.0 && boxes[i].label != 0) {
                syslog(
                    LOG_INFO,
                    "Object %d: Classes: %s - Scores: %f - Locations: [t%f,l%f,b%f,r%f] normalized "
                    "locations [%f,%f,%f,%f]",
                    i,
